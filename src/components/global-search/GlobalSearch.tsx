...
import { listChapters, readChapter, updateProjectMeta } from '@/lib/project-fs';
...
function GlobalSearch() {
  ...
  const setShowGlobalSearch = useLayoutStore((s) => s.setShowGlobalSearch);
  ...
  useEffect(() => {
    ...
    async function load() {
      const result = await listChapters(currentProject!);
      if (!result.ok) {
        setLoading(false);
        return;
      }
      const loaded: ChapterContent[] = [];
      for (const ch of result.value) {
        const read = await readChapter(ch.path);
        if (read.ok) {
          loaded.push({ chapter: ch, lines: read.value.split('\n') });
        }
      }
      setContents(loaded);
      setLoading(false);
