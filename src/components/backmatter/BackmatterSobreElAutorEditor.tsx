...
import { readSobreElAutor, writeSobreElAutor } from '@/lib/backmatter-fs';
import { commitChanges } from '@/lib/versioning';
import ChapterEditor from '@/components/editor/ChapterEditor';

function BackmatterSobreElAutorEditor() {
  ...
  useEffect(() => {
    ...
    readSobreElAutor(currentProject.rootPath).then((data) => {
  ...
  function handleChange(content: string) {
    ...
    timerRef.current = setTimeout(async () => {
      ...
      const filePath = `${currentProject.rootPath}/backmatter/sobre-el-autor.md`;
      await writeSobreElAutor(currentProject.rootPath, { contenido: content });
      lastSavedRef.current = content;
    ...
    <div className="h-full flex flex-col bg-bg-editor">
      <div className="flex items-center justify-between px-3 py-1.5 border-b border-border-subtle shrink-0">
        <span className="text-xs text-text-tertiary font-sans uppercase tracking-wider">
          {t('backmatter.sobreElAutor.sectionTitle')}
        </span>
        {saveStatus === 'saving' && (
...
export default BackmatterSobreElAutorEditor;
