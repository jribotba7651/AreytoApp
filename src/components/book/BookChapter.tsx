...
const MD_COMPONENTS: Components = {
  ...
  blockquote: ({ children }) => (
    <blockquote
      className="border-l-[3px] border-accent-muted pl-4 italic text-text-secondary my-6"
    >
      {children}
    </blockquote>
  ),
  code: ({ children, className }) => {
    ...
    if (isBlock) {
      return (
        <code className="font-mono text-sm text-text-editor">
          {children}
        </code>
      );
    }
  ...
  hr: () => (
    <hr className="border-border-subtle my-8 mx-auto w-16" />
  ),
};
