...
function ExportBookDialog({ onClose, onExport, loading }: ExportBookDialogProps) {
    ...
    >
      <div className="bg-bg-tertiary border border-border-default rounded-lg p-6 w-full max-w-md mx-4">
        <h2 className="text-base font-medium text-text-primary mb-1">
          {t('modal.exportMarkdown.title')}
        </h2>
        <p className="text-sm text-text-secondary mb-4">
          {t('modal.exportMarkdown.description')}
        </p>
        ...
        <div className="flex justify-end gap-2">
          ...
          >
            {loading ? t('modal.exportMarkdown.exporting') : t('modal.exportMarkdown.export')}
          </button>
        </div>
      </div>
    </div>
...
export default ExportBookDialog;
