...
function ExportAllDialog({ onClose, onExport, loading }: ExportAllDialogProps) {
    ...
    >
      <div className="bg-bg-tertiary border border-border-default rounded-lg p-6 w-full max-w-md mx-4">
        <h2 className="text-base font-medium text-text-primary mb-1">
          {t('modal.exportAll.title')}
        </h2>
        <p className="text-sm text-text-secondary mb-4">
          {t('modal.exportAll.description')}
        </p>
        ...
        <div className="flex justify-end gap-2">
          ...
          >
            {loading ? t('modal.exportAll.exporting') : t('modal.exportAll.export')}
          </button>
        </div>
      </div>
    </div>
